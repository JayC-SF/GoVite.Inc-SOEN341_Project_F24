...
export function TeacherCourseGroups(props: TeacherCourseGroupsProps) {
  ...
  const onSaveNewGroup = async () => {
    ...
    await PostCreateNewGroup({
      groupname: newGroupName || "",
      courseid: props.courseInfo?.course.courseid || "",
    });
    props.refreshGroups();
  };
      ...
      <div>
        ...
        {newGroupName != undefined && (
          <div className="flex gap-2">
            <input
              className="border border-gray-300 text-center p-1 mb-3"
              onChange={(e) => setNewGroupName(e.target.value)}
              placeholder="New Group Name"
            />
            <div className="flex gap-1">
              <RMPButton onClick={() => setNewGroupName(undefined)}>
                Cancel
              </RMPButton>
              <RMPButton onClick={onSaveNewGroup}>Save</RMPButton>
            </div>
          </div>
        )}
      </div>
